...
def calculate_rebalance(holdings: dict, current_prices: dict, target_weights: dict) -> dict:
    ...
    for token_id, amount in holdings.items():
        ...
        portfolio[token_id] = {"amount": amount, "price_usd": price,
                               "value_usd": value, "change_24h_pct": round(change_24h, 2)}
        total_value += value
    ...
    return {"total_value_usd": round(total_value, 2), "portfolio": portfolio,
            "rebalance_actions": actions, "actions_needed": len(actions)}


def register_autopilot_tools(app: FastMCP):
    @app.tool()
    async def autopilot_analyze(holdings: dict, target_weights: dict,
                                notify_telegram: bool = True) -> dict:
            ...
            if notify_telegram and result["actions_needed"] > 0:
                ...
                for a in result["rebalance_actions"]:
                    ...
                    actions_text += (f"{emoji} <b>{a['action']} {a['token'].upper()}</b>\n"
                                    ...
                                    f"   {a['current_weight_pct']}% → {a['target_weight_pct']}% [{a['urgency']}]\n\n")
                message = (f"📊 <b>Portfolio Autopilot Report</b>\n\n"
                           ...
                           f"{actions_text}🤖 Powered by Scutua-MCP")
