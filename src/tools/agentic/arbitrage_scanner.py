...
# Rough bridge + gas cost estimates in USD
BRIDGE_COSTS = {
...
async def fetch_price(token_id: str) -> dict:
    """Fetch token price across chains via CoinGecko."""
    params = {
        "ids": token_id,
        "vs_currencies": "usd",
...
async def scan_arbitrage(
    ...
    Returns:
        List of opportunities with net profit after bridge + gas fees
    """
    try:
        ...
        # Simulate cross-chain price spread (real impl would use DEX APIs per chain)
        # Using ±0.3-1.5% spread as realistic DEX slippage simulation
        import random
        random.seed(42)
        ...
        for i, chain in enumerate(chain_list):
            ...
            chain_prices[chain] = round(price_usd * (1 + spread), 6)
