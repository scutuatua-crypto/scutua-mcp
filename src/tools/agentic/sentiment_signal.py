...
async def fetch_market_data(token_ids: list[str]) -> dict:
    ...
    if cached:
        return cached
    params = {"ids": ",".join(token_ids), "vs_currencies": "usd", "include_24hr_change": "true", "include_24hr_vol": "true", "include_market_cap": "true"}
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        ...
        if resp.status_code == 429:
            ...
            return stale[0] if stale else {"error": "CoinGecko rate limit"}
        resp.raise_for_status()
        data = resp.json()
...
def calculate_signal(fear_greed_value: int, price_change_24h: float, volume_change_pct: float = 0) -> dict:
    ...
    return {"signal": signal, "confidence": confidence, "emoji": emoji, "composite_score": round(composite, 1), "components": {"fear_greed_score": fg_score, "price_momentum_score": round(price_score, 1), "volume_score": round(vol_score, 1)}}
...
async def send_telegram(message: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        resp = await client.post(url, json={"chat_id": TELEGRAM_CHANNEL_ID, "text": message, "parse_mode": "HTML"})
        resp.raise_for_status()
        return {"sent": True}


async def get_sentiment_signal(tokens: list[str] = None, notify_telegram: bool = True) -> dict:
    """Generate unified BUY/SELL/HOLD signal from on-chain sentiment data."""
    if tokens is None:
        tokens = ["bitcoin", "ethereum", "solana"]
    try:
        fg_data, market_data = await asyncio.gather(fetch_fear_greed(), fetch_market_data(tokens))
        ...
        for token_id in tokens:
            ...
            signals[token_id] = {"price_usd": td.get("usd", 0), "change_24h_pct": round(price_change, 2), **sig}

        if not signals:
            return {"error": "No signal data available"}

        avg_composite = sum(s["composite_score"] for s in signals.values()) / len(signals)
        market_signal = "🟢 BULLISH" if avg_composite >= 65 else "🔴 BEARISH" if avg_composite <= 35 else "🟡 NEUTRAL"
        result = {"fear_greed": fg_data, "market_signal": market_signal, "market_composite_score": round(avg_composite, 1), "signals": signals}

        if notify_telegram:
            lines = [f"{s['emoji']} <b>{tid.upper()}</b>: {s['signal']} ({s['confidence']}) | ${s['price_usd']:,.2f} ({s['change_24h_pct']:+.2f}%)" for tid, s in signals.items()]
            message = f"📡 <b>Sentiment Signal Report</b>\n\n😨 Fear & Greed: <b>{fg_data['value']} — {fg_data['classification']}</b>\n🌍 Market: <b>{market_signal}</b>\n\n" + "\n".join(lines) + "\n\n🤖 Scutua-MCP Agentic Layer"
